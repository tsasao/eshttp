...
const PARSER_MAX_TRAILERS_LENGTH = 1024 * 1; /* 1 KiB */
...
var PARSER_STATE_HTTP_TRAILERS = 24;
var PARSER_STATE_HTTP_TRAILER_KEY = 25;
var PARSER_STATE_HTTP_TRAILER_VALUE = 26;
...
class HttpParser {
  ...
  get trailers() {
    return this._trailers ? this._trailers : new Headers();
  }
  ...
  _addTrailer(name, value) {
    name = name.toLowerCase();
    value = value.trimRight();

    // lazy-create trailers object
    if (!this._trailers) {
      this._trailers = new Headers();
    }

    // push directly into private state to avoid possible validation
    this._trailers._names.push(name);
    this._trailers._values.push(value);
  }
  ...
  chunk(u8, offset) {
    ...
    for (var i = offset, l = u8.length; i < l; ++i) {
      ...
      if (this._bodyReceived && ++this._headersLength > PARSER_MAX_TRAILERS_LENGTH) {
        this._error();
        return;
      }
      ...
      case PARSER_STATE_BODY_CHUNK:
        var cur = u8.subarray(i, Math.min(i + this._chunkLength, u8.length));
        if(this._lastBodyChunk === null) {
          this._lastBodyChunk = cur;
        } else {
          this._lastBodyChunk = (function(a, b) {
      ...
      case PARSER_STATE_BODY_CHUNK_SIZE:
        if (c === '\n') {
          this._chunkLength = parseInt(this._tmpStringKey, 16);
          if (this._chunkLength === 0) {
            this._tmpStringKey = '';
            this._headersLength = 0;
            this._bodyReceived = true;
            this._state = PARSER_STATE_HTTP_TRAILERS;
          } else {
      ...
      case PARSER_STATE_BODY_CHUNK_NEXT:
        ...
        this._error();
        return;
      case PARSER_STATE_HTTP_TRAILERS:
        if (c === '\n') {
          if (this._tmpStringKey) {
            this._addTrailer(this._tmpStringKey, this._tmpStringValue);
          }
        ...
        if (c === ' ') {
          ...
          this._state = PARSER_STATE_HTTP_TRAILER_VALUE;
          continue;
        }
        ...
        this._tmpStringKey = c;
        this._state = PARSER_STATE_HTTP_TRAILER_KEY;
        continue;
      case PARSER_STATE_HTTP_TRAILER_KEY:
        if (c === ':') {
          this._state = PARSER_STATE_HTTP_TRAILER_VALUE;
          this._tmpStringValue = '';
          continue;
        ...
        this._tmpStringKey += c;
        continue;
      case PARSER_STATE_HTTP_TRAILER_VALUE:
        if (c === '\n') {
          this._state = PARSER_STATE_HTTP_TRAILERS;
          continue;
        }

        // skip spaces before trailer value
        // "x-trailer:   value" => "x-trailer" = "value"
        if (c === ' ' && this._tmpStringValue.length === 0) {
          continue;
        }
